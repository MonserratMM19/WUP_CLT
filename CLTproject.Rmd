...
The previous graph shows that the mean is getting closer and closer to 50 and the standard deviation is decreasing which is by CLT theory. The standard deviation was expected, because the sample standard deviation was 41.8671520098683 and when divided by square root 9 (the sample size) it equals approximately 13.95571734, which is close to the results of approximately 14. overall, the graph is getting more symmetrical. 
...
print(paste('mean = ', mean(my1000$k16),' sd = ',sd(my1000$k)))

```

The graph is looking more symmetrical because now we are using 16 samples at a time which gives more chance for opinion discrepancies. and the standard deviation is decreasing as expected because we are dividing by a larger number. in this case, it's the square root of 16 which equals 4. and its closer to our expected value which is 10.466788
...
The graph is looking more symmetrical, because now we are using 25 samples at a time which gives for more chances for opinion discrepancies. and the standard deviation is decreasing as expected because we are dividing by a larger number. in this case it's the square root of 25. and its closer to our expected value which is 8.373430402
...
print(paste('mean = ', mean(my1000$k36),' sd = ',sd(my1000$k)))

```
The graph is looking more symmetrical because now we are using 36 samples at a time which gives more chance for opinion discrepancies. and the standard deviation is decreasing as expected because we are dividing by a larger number in this case it's the square root of 36. and its closer to our expected value which is 6.977858668. This graph, in addition to the previous ones, demonstrated the central limit theorem by showing standard deviation as we increase the sample size. and its closer to our expected value which is 
